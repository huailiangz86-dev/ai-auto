...
export class OperationsWorkbenchService {
  constructor(
    ...
    private readonly financialLedgerRepo: Repository<FinancialLedgerEntry>,
  ...
  async listReviewQueue(query: OperationsQueueQueryDto) {
    ...
    return this.listQueue(
      query,
      statuses.filter((status) => REVIEW_STATUSES.includes(status)),
    )
  }
  ...
  async listRiskQueue(query: OperationsQueueQueryDto) {
    ...
    return this.listQueue(
      query,
      statuses.filter((status) => RISK_STATUSES.includes(status)),
    )
  }
  ...
  async getTaskDetail(creatorTaskId: string) {
    ...
    const [growthTask, creditLedger, financialEntries, evidence, auditRecords, notifications] =
      await Promise.all([
        ...
        this.financialLedgerRepo.find({
          where: { creatorTaskId: task.id },
          order: { occurredAt: 'DESC', createdAt: 'DESC' },
        }),
        this.contentRepo.find({ where: { creatorTaskId: task.id }, order: { createdAt: 'DESC' } }),
        this.auditRepo.find({
          where: { targetType: 'creator_task', targetId: task.id },
          order: { createdAt: 'DESC' },
        }),
        this.notificationRepo.find({
          where: { recipientId: task.creatorId, targetType: 'creator_task', targetId: task.id },
          order: { createdAt: 'DESC' },
        }),
      ])
    ...
    const publications = evidenceIds.length
      ? await this.publicationRepo.find({
          where: { contentId: In(evidenceIds) },
          order: { createdAt: 'DESC' },
        })
      : []
    ...
    return {
      ...
      economics: {
        compensation: {
          ...
          lockedSnapshot: task.compensationSnapshot,
          lockedAt: task.compensationLockedAt,
        },
        campaignCredits: {
          allocated,
          consumed,
      ...
      evidence: evidence.map((item) => ({
        ...item,
        creatorStudioAction:
          item.contentType === 'creator_studio' ? (item.contentData?.action ?? null) : null,
  ...
  private async listQueue(query: OperationsQueueQueryDto, statuses: CreatorTaskStatus[]) {
    ...
    const builder = this.creatorTaskRepo
    ...
    const [items, total] = await builder
      ...
      .getManyAndCount()
    return {
      items: items.map((task) => this.toTaskSummary(task)),
  ...
  private applyScope(builder: SelectQueryBuilder<CreatorTask>, query: OperationsQueueQueryDto) {
    if (query.creatorTaskId)
      builder.andWhere('task.id = :creatorTaskId', { creatorTaskId: query.creatorTaskId })
    if (query.campaignId)
      builder.andWhere('task.campaignId = :campaignId', { campaignId: query.campaignId })
    if (query.merchantId)
      builder.andWhere('task.merchantId = :merchantId', { merchantId: query.merchantId })
    if (query.creatorId)
      builder.andWhere('task.creatorId = :creatorId', { creatorId: query.creatorId })
    if (query.growthTaskId)
      builder.andWhere('task.growthTaskId = :growthTaskId', { growthTaskId: query.growthTaskId })
  }
  ...
  private toTaskSummary(task: CreatorTask) {
    ...
    return {
      ...
      review: {
        reason: task.reviewReason,
        reviewedBy: task.reviewedBy,
        reviewedAt: task.reviewedAt,
      },
      risk: {
        holdReason: task.riskHoldReason,
        previousStatus: task.riskHoldPreviousStatus,
        resolutionReason: task.stateReason,
        changedBy: task.stateChangedBy,
        changedAt: task.stateChangedAt,
      },
      economics: {
        baseReward: Number(task.baseReward),
        compensationLockedAt: task.compensationLockedAt,
        campaignCreditsAllocated: allocated,
        campaignCreditsConsumed: consumed,
        campaignCreditsRemaining: allocated - consumed,
      },
