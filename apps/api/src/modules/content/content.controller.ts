...
export class ContentController {
  constructor(
    ...
    private readonly creatorStudioService: CreatorStudioService,
  ) {}
  ...
  // ========================
  // Creator Studio（CREATOR-V2-004）
  // ========================

  @Post('creator-studio/generate')
  @Roles(UserRole.AGENT)
  @ApiOperation({ summary: '使用任务 Campaign Credits 生成内容' })
  async studioGenerate(@CurrentUser() user: { agentId: string }, @Body() dto: CreatorStudioGenerateDto) {
    return this.creatorStudioService.generate(user.agentId, dto)
  }

  @Post('creator-studio/rewrite')
  @Roles(UserRole.AGENT)
  @ApiOperation({ summary: '使用任务 Campaign Credits 改写内容' })
  async studioRewrite(@CurrentUser() user: { agentId: string }, @Body() dto: CreatorStudioRewriteDto) {
    return this.creatorStudioService.rewrite(user.agentId, dto)
  }

  @Post('creator-studio/score')
  @Roles(UserRole.AGENT)
  @ApiOperation({ summary: '使用任务 Campaign Credits 评分内容' })
  async studioScore(@CurrentUser() user: { agentId: string }, @Body() dto: CreatorStudioScoreDto) {
    return this.creatorStudioService.score(user.agentId, dto)
  }

  @Post('creator-studio/publish-advice')
  @Roles(UserRole.AGENT)
  @ApiOperation({ summary: '使用任务 Campaign Credits 获取发布建议' })
  async studioPublishAdvice(
    @CurrentUser() user: { agentId: string },
    @Body() dto: CreatorStudioPublishAdviceDto,
  ) {
    return this.creatorStudioService.publishAdvice(user.agentId, dto)
  }
  ...
  async getVideoStatus(
    ...
    return this.videoService.getJobStatus(contentId, user.agentId)
  }
