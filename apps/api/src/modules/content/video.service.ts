...
export class VideoService {
  ...
  async generateVideo(
    ...
    const { couponId, campaignId, platform, durationSeconds = 30 } = params
   ...
   * SSE 进度端点用的任务状态查询
   */
  async getJobStatus(contentId: string): Promise<VideoJobStatus> {
  ...
  private async processVideoJob(
    ...
    try {
      // Stage 1: 开始生成 (50%)
      await this.updateJobData(contentId, { jobStatus: 'generating', progress: 50 })
      await this.publishProgress(contentId, { status: 'generating', progress: 50 })
      ...
      const thumbnailUrl = videoResult?.thumbnail_url ?? videoResult?.thumbnailUrl ?? null
      const script = videoResult?.script ?? videoResult?.content ?? null
      ...
      this.logger.log({ event: 'video_job_completed', contentId, videoUrl })
    } catch (error) {
      await this.updateJobData(contentId, {
        jobStatus: 'failed',
        progress: 0,
