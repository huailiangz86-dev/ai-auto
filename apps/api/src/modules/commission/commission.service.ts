...
export class CommissionService {
  ...
  private async calculateAndRecord(
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      createdCommissionId = savedCommission.id
    ...
    return { commissionId: createdCommissionId, agentPayout: agentFinalPayout }
  }
  ...
  async redeemCoupon(merchantId: string, dto: RedeemCouponDto): Promise<RedeemResultDto> {
    ...
    await this.customerCouponRepo.update(
      ...
      { status: CouponStatus.REDEEMED, usedAt: now, redemptionId: savedRedemption.id },
    )
    ...
    await this.taskService.trackRedemption(
      customerCoupon.agentId,
