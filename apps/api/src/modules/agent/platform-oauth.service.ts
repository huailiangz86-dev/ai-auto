...
export class PlatformOAuthService {
  ...
  buildAuthorizeUrl(platformType: PlatformType, redirectUri: string, state: string): string {
    if (platformType === PlatformType.KUAISHOU) {
      const params = new URLSearchParams({
        app_id: this.getClientId(platformType),
  ...
  async exchangeCodeForToken(
    ...
    if (platformType === PlatformType.KUAISHOU) {
      const data = await this.getWithParams<Record<string, unknown>>(
        'https://open.kuaishou.com/oauth2/access_token',
        {
          app_id: this.getClientId(platformType),
          app_secret: this.getClientSecret(platformType),
          grant_type: 'authorization_code',
          code,
        },
        platformType,
      )
      this.assertKuaishouSuccess(data)
  ...
  async getUserInfo(platformType: PlatformType, accessToken: string): Promise<PlatformUserInfo> {
    if (platformType === PlatformType.KUAISHOU) {
      const data = await this.getWithParams<Record<string, unknown>>(
        'https://open.kuaishou.com/openapi/user_info',
        { app_id: this.getClientId(platformType), access_token: accessToken },
        platformType,
      )
      this.assertKuaishouSuccess(data)
      const user = (data['user_info'] ?? data) as Record<string, unknown>
      return {
        platformUserId: String(user['open_id'] ?? user['openId'] ?? ''),
        nickname: String(user['name'] ?? user['nickname'] ?? ''),
        avatar: user['head'] ? String(user['head']) : undefined,
        isEnterpriseAccount: Boolean(user['is_enterprise'] ?? false),
      }
    }
  ...
  private getClientId(platformType: PlatformType): string {
    const map: Record<string, string> = {
      ...
      [PlatformType.KUAISHOU]: this.config.get('oauth.kuaishou.clientId', ''),
    }
  ...
  private getClientSecret(platformType: PlatformType): string {
    const map: Record<string, string> = {
      ...
      [PlatformType.KUAISHOU]: this.config.get('oauth.kuaishou.clientSecret', ''),
    }
    return map[platformType] ?? ''
  }
  ...
  private async getWithParams<T>(
    url: string,
    params: Record<string, string>,
    platform: PlatformType,
  ): Promise<T> {
    const response = await lastValueFrom(
      this.httpService.get<T>(url, { params, timeout: 10000 }).pipe(
        catchError((err) => {
          this.logger.error({
            event: 'oauth_get_error',
            platform,
  ...
  private toTokens(data: Record<string, unknown>): OAuthTokens {
    return {
      ...
      platformUserId: data['open_id'] ? String(data['open_id']) : undefined,
    }
  }

  private assertKuaishouSuccess(data: Record<string, unknown>) {
    if (String(data['result']) !== '1') {
      throw new Error(`Kuaishou OAuth failed: ${String(data['error_msg'] ?? 'unknown error')}`)
    }
  }
}
