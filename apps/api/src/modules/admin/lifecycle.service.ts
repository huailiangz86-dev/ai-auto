...
export class LifecycleService {
  ...
  async listMerchants(query: ListLifecycleSubjectsDto) {
    ...
    if (query.keyword)
      qb.andWhere(
        '(merchant.id::text ILIKE :keyword OR merchant.business_name ILIKE :keyword OR merchant.phone ILIKE :keyword)',
    ...
    if (query.auditStatus)
      qb.andWhere('merchant.audit_status = :auditStatus', { auditStatus: query.auditStatus })
    ...
    return this.paginate(
      await Promise.all(merchants.map((merchant) => this.merchantListItem(merchant))),
      page,
      pageSize,
      total,
    )
  ...
  async listCreators(query: ListLifecycleSubjectsDto) {
    ...
    if (query.keyword)
      qb.andWhere(
        '(creator.id::text ILIKE :keyword OR creator.nickname ILIKE :keyword OR creator.phone ILIKE :keyword OR creator.wechat_openid ILIKE :keyword OR creator.wechat_unionid ILIKE :keyword)',
        { keyword: `%${query.keyword.trim()}%` },
      )
    if (query.status === 'active')
      qb.andWhere('creator.status = true AND creator.blacklisted_at IS NULL')
    if (query.status === 'frozen')
      qb.andWhere('creator.status = false AND creator.blacklisted_at IS NULL')
    if (query.status === 'blacklisted') qb.andWhere('creator.blacklisted_at IS NOT NULL')
    if (query.agentType)
      qb.andWhere('creator.agent_type = :agentType', { agentType: query.agentType })
    const [creators, total] = await qb
      .orderBy('creator.updatedAt', 'DESC')
      .skip((page - 1) * pageSize)
      .take(pageSize)
      .getManyAndCount()
    return this.paginate(
      await Promise.all(creators.map((creator) => this.creatorListItem(creator))),
      page,
      pageSize,
      total,
    )
  ...
  async getCreatorDetail(creatorId: string) {
    ...
    const [summary, accounts, contents, publications, notes, relationships, audit] =
      await Promise.all([
        this.creatorSummary(creator),
        this.dataSource
          .getRepository(AgentPlatformAccount)
          .find({ where: { agentId: creatorId }, order: { boundAt: 'DESC' } }),
        this.dataSource
          .getRepository(Content)
          .find({ where: { agentId: creatorId }, order: { createdAt: 'DESC' }, take: 100 }),
        this.dataSource
    ...
    return {
      ...
      contents: contents.map((content) =>
        this.creatorContentItem(content, publicationsByContent.get(content.id) ?? []),
      ),
      notes,
      relationships,
      audit,
  ...
  async freezeMerchant(merchantId: string, dto: LifecycleReasonDto, actor: Actor) {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      await this.writeAudit(
        manager,
        actor,
        AuditActionType.MERCHANT_FROZEN,
        '商户已冻结',
        'merchant',
        merchant.id,
  ...
  async restoreMerchant(merchantId: string, dto: RestoreLifecycleDto, actor: Actor) {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      await this.writeAudit(
        manager,
        actor,
        AuditActionType.MERCHANT_RESTORED,
        '商户已恢复',
        'merchant',
        merchant.id,
        merchant.businessName,
      ...
      await this.writeNotification(
        ...
        'merchant_lifecycle',
        '商户账号已恢复',
        dto.reason ? `恢复说明：${dto.reason}` : '你的账号已恢复正常使用。',
        'merchant',
        merchant.id,
      )
    })
  ...
  async restoreCreator(creatorId: string, dto: RestoreLifecycleDto, actor: Actor) {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      await this.writeAudit(
        manager,
        actor,
        AuditActionType.CREATOR_RESTORED,
        '达人已恢复',
        'creator',
        creator.id,
        creator.nickname,
        { reason: dto.reason ?? null, blacklistCleared: true },
      )
      await this.writeNotification(
        manager,
        creator.id,
  ...
  async setMerchantTags(merchantId: string, dto: SetLifecycleTagsDto, actor: Actor) {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      await this.writeAudit(
        manager,
        actor,
        AuditActionType.LIFECYCLE_TAGGED,
        '商户运营标签已更新',
        'merchant',
        merchant.id,
        merchant.businessName,
  ...
  async setCreatorTags(creatorId: string, dto: SetLifecycleTagsDto, actor: Actor) {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      await this.writeAudit(
        ...
        creator.id,
        creator.nickname,
        { tags: creator.operationTags },
      )
    })
    return { code: 0, data: { tags: creator.operationTags } }
  }

  async setCreatorType(
    creatorId: string,
  ...
  async setCreatorTaskLimit(creatorId: string, dto: SetCreatorTaskLimitDto, actor: Actor) {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      await this.writeAudit(
        manager,
        actor,
        AuditActionType.CREATOR_TASK_LIMIT_UPDATED,
        '达人限接任务设置已更新',
        'creator',
        creator.id,
        creator.nickname,
        { limit: creator.creatorTaskLimit },
      )
    })
  ...
  async createNote(
    ...
    subjectId: string,
    dto: CreateLifecycleNoteDto,
    actor: Actor,
  ) {
    let targetName: string | null | undefined
    if (subjectType === 'merchant')
      targetName = (await this.requireMerchant(subjectId)).businessName
    ...
    const note = await this.dataSource.transaction(async (manager) => {
      const saved = await manager.save(LifecycleNote, {
        subjectType,
        subjectId,
      ...
      await this.writeAudit(
        manager,
        actor,
        AuditActionType.LIFECYCLE_NOTE_CREATED,
        '运营跟进记录已新增',
        subjectType,
        subjectId,
        targetName,
        { category: dto.category, reason: dto.reason ?? null, followUpAt: dto.followUpAt ?? null },
      )
      return saved
    })
  ...
  async notify(
    subjectType: 'merchant' | 'creator',
    subjectId: string,
    dto: SendLifecycleNotificationDto,
    actor: Actor,
  ) {
    let targetName: string | null | undefined
    if (subjectType === 'merchant')
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      await this.writeAudit(
        manager,
        actor,
        AuditActionType.LIFECYCLE_NOTIFICATION_SENT,
        '运营站内通知已发送',
        subjectType,
        subjectId,
        targetName,
  ...
  async listRelationships(merchantId?: string, creatorId?: string) {
    return this.relationships({ merchantId, creatorId })
  }
  ...
  async restrictRelationship(bindingId: string, dto: LifecycleReasonDto, actor: Actor) {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      await this.writeAudit(
        manager,
        actor,
        AuditActionType.RELATIONSHIP_RESTRICTED,
        '商户—达人合作已限制',
        'relationship',
        binding.id,
        binding.inviteCode,
        { merchantId: binding.merchantId, creatorId: binding.agentId ?? null, reason: dto.reason },
      )
    })
  ...
  async releaseRelationship(bindingId: string, dto: RestoreLifecycleDto, actor: Actor) {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      await this.writeAudit(
        manager,
        actor,
        AuditActionType.RELATIONSHIP_RELEASED,
        '商户—达人合作限制已解除',
        'relationship',
        binding.id,
        binding.inviteCode,
  ...
  async unbindRelationship(bindingId: string, dto: LifecycleReasonDto, actor: Actor) {
    ...
    if (binding.bindingStatus === 'unbound')
      throw new BadRequestException({ code: 6501, message: '该合作关系已解除' })
    await this.dataSource.transaction(async (manager) => {
      ...
      await this.writeAudit(
        manager,
        actor,
        AuditActionType.RELATIONSHIP_UNBOUND,
        '商户—达人合作已人工解除',
        'relationship',
        binding.id,
        binding.inviteCode,
  ...
  private async merchantListItem(merchant: Merchant) {
    return { ...this.merchantProfile(merchant), summary: await this.merchantSummary(merchant) }
  }
  private async creatorListItem(creator: SharingAgent) {
    return { ...this.creatorProfile(creator), summary: await this.creatorSummary(creator) }
  }
  ...
  private creatorProfile(creator: SharingAgent) {
    return {
      ...
      wechatOpenid: creator.wechatOpenid ?? null,
      wechatOpenidMasked: this.maskWechatId(creator.wechatOpenid),
      wechatUnionid: creator.wechatUnionid ?? null,
      auditStatus: creator.auditStatus,
      agentType: creator.agentType ?? 'ordinary_user',
      realNameVerified: creator.realNameVerified,
      level: creator.level,
      growthScore: creator.creatorGrowthScore ?? 0,
      growthLevel: creator.creatorGrowthLevel ?? 1,
      categories: creator.creatorCategories ?? [],
      taskPreferences: creator.taskPreferences ?? {},
      status: creator.blacklistedAt ? 'blacklisted' : creator.status ? 'active' : 'frozen',
      frozenAt: creator.frozenAt ?? null,
      frozenReason: creator.frozenReason ?? null,
      blacklistedAt: creator.blacklistedAt ?? null,
      blacklistReason: creator.blacklistReason ?? null,
  ...
  private async merchantSummary(merchant: Merchant) {
    const [campaigns, tasks, creatorTasks, redemptions, commissions, subscriptions, alerts] =
      await Promise.all([
        this.dataSource
        ...
        this.dataSource
          .getRepository(GrowthTask)
          .find({ where: { merchantId: merchant.id }, select: ['id', 'status', 'budget'] }),
        this.dataSource
          .getRepository(CreatorTask)
          .find({
            where: { merchantId: merchant.id },
            select: ['id', 'status', 'campaignCreditsAllocated', 'campaignCreditsConsumed'],
          }),
        this.dataSource
          .getRepository(Redemption)
          .find({
            where: { merchantId: merchant.id },
            select: ['id', 'status', 'transactionAmount'],
          }),
        this.dataSource
          .getRepository(Commission)
          .find({
            where: { merchantId: merchant.id },
            select: ['id', 'status', 'agentFinalPayout'],
          }),
        this.dataSource
          .getRepository(Subscription)
          .find({ where: { merchantId: merchant.id }, order: { createdAt: 'DESC' }, take: 1 }),
        this.dataSource
    ...
    return {
      activity: {
        ...
        activeCampaigns: campaigns.filter((item) => item.campaignStatus === 'active').length,
        growthTasks: tasks.length,
        activeGrowthTasks: tasks.filter((item) => item.status === 'active').length,
      },
      budget: {
        planned: this.sum(campaigns, 'maxBudget') + this.sum(tasks, 'budget'),
        spent: this.sum(campaigns, 'spentBudget'),
        commissionSpent: this.sum(campaigns, 'totalCommissionSpent'),
      },
      settlement: {
        redemptions: redemptions.length,
        verifiedRedemptions: redemptions.filter(
          (item) => item.status === 'verified' || item.status === 'settled',
        ).length,
        gmv: this.sum(redemptions, 'transactionAmount'),
        commission: this.sum(commissions, 'agentFinalPayout'),
        pendingCommission: this.sum(
          commissions.filter((item) => item.status === 'pending' || item.status === 'frozen'),
          'agentFinalPayout',
        ),
      },
      creatorTasks: {
        total: creatorTasks.length,
        inProgress: creatorTasks.filter(
          (item) =>
            !['completed', 'settled', 'cancelled', 'rejected', 'violation', 'expired'].includes(
              item.status,
            ),
        ).length,
        completed: creatorTasks.filter((item) => ['completed', 'settled'].includes(item.status))
          .length,
      },
      credits: {
        allocated: this.sum(creatorTasks, 'campaignCreditsAllocated'),
        consumed: this.sum(creatorTasks, 'campaignCreditsConsumed'),
      },
      risk: {
      ...
      subscription: subscriptions[0]
        ? {
            ...
            startAt: subscriptions[0].startAt,
            expireAt: subscriptions[0].expireAt,
          }
        : null,
    }
  }
  ...
  private async creatorSummary(creator: SharingAgent) {
    const [creatorTasks, assignments, contents, publications, commissions, bindings, alerts] =
      await Promise.all([
        this.dataSource
          .getRepository(CreatorTask)
          .find({ where: { creatorId: creator.id }, select: ['id', 'status', 'channel'] }),
        this.dataSource
          .getRepository(SharingTaskAssignment)
          .find({
            where: { agentId: creator.id },
            select: ['id', 'status', 'viewCount', 'claimCount', 'redemptionCount', 'earnedReward'],
          }),
        this.dataSource
          .getRepository(Content)
          .find({
            where: { agentId: creator.id },
            select: ['id', 'status', 'totalImpressions', 'totalClicks', 'totalClaims'],
          }),
        this.dataSource
          .getRepository(ContentPublication)
          .find({
            where: { agentId: creator.id },
            select: ['id', 'status', 'impressions', 'clicks'],
          }),
        this.dataSource
          .getRepository(Commission)
          .find({ where: { agentId: creator.id }, select: ['id', 'agentFinalPayout'] }),
        this.dataSource
          .getRepository(MerchantAgentBinding)
          .find({
            where: { agentId: creator.id },
            select: ['id', 'bindingStatus', 'restrictedAt'],
          }),
        this.dataSource
          .getRepository(FraudAlert)
          .find({ where: { agentId: creator.id, status: 'pending' }, select: ['id', 'severity'] }),
      ])
    const taskTotal = creatorTasks.length + assignments.length
    const taskCompleted =
      creatorTasks.filter((item) => ['completed', 'settled'].includes(item.status)).length +
      assignments.filter((item) => item.status === 'completed').length
    return {
      taskPerformance: {
        total: taskTotal,
        completed: taskCompleted,
        fulfillmentRate: taskTotal ? Number((taskCompleted / taskTotal).toFixed(4)) : 0,
        current: creatorTasks.filter(
          (item) =>
            !['completed', 'settled', 'cancelled', 'rejected', 'violation', 'expired'].includes(
              item.status,
            ),
      ...
      conversion: {
        views: this.sum(assignments, 'viewCount'),
        claims: this.sum(assignments, 'claimCount') + this.sum(contents, 'totalClaims'),
        redemptions: this.sum(assignments, 'redemptionCount'),
        earnedReward: this.sum(assignments, 'earnedReward'),
        commissionEarned: this.sum(commissions, 'agentFinalPayout'),
      },
      publishing: {
        ...
        impressions: Math.max(
          this.sum(contents, 'totalImpressions'),
          this.sum(publications, 'impressions'),
        ),
        clicks: Math.max(this.sum(contents, 'totalClicks'), this.sum(publications, 'clicks')),
      },
      relationships: {
        total: bindings.length,
        active: bindings.filter((item) => item.bindingStatus === 'active').length,
        restricted: bindings.filter((item) => Boolean(item.restrictedAt)).length,
      },
      risk: {
        pendingAlerts: alerts.length,
        criticalAlerts: alerts.filter((item) => item.severity === 'critical').length,
  ...
  private async relationships(filter: { merchantId?: string; creatorId?: string }) {
    ...
    const creatorIds = [
      ...new Set(bindings.flatMap((item) => (item.agentId ? [item.agentId] : []))),
    ]
    const [merchants, creators, tasks] = await Promise.all([
      merchantIds.length
        ? this.dataSource.getRepository(Merchant).findBy({ id: In(merchantIds) })
        : [],
      creatorIds.length
        ? this.dataSource.getRepository(SharingAgent).findBy({ id: In(creatorIds) })
        : [],
      creatorIds.length && merchantIds.length
        ? this.dataSource
            .getRepository(CreatorTask)
            .find({ where: creatorIds.map((creatorId) => ({ creatorId })) })
        : [],
    ])
    const merchantMap = new Map<string, Merchant>(
      merchants.map((item): [string, Merchant] => [item.id, item]),
    )
    const creatorMap = new Map<string, SharingAgent>(
      creators.map((item): [string, SharingAgent] => [item.id, item]),
    )
    return bindings.map((binding) => ({
      id: binding.id,
      merchant: merchantMap.get(binding.merchantId)
        ? {
            id: binding.merchantId,
            businessName: merchantMap.get(binding.merchantId)?.businessName,
          }
        : { id: binding.merchantId, businessName: '已删除商户' },
      creator:
        binding.agentId && creatorMap.get(binding.agentId)
          ? {
              id: binding.agentId,
              nickname: creatorMap.get(binding.agentId)?.nickname,
              phone: this.maskPhone(creatorMap.get(binding.agentId)?.phone ?? ''),
            }
          : null,
      bindingStatus: binding.bindingStatus,
      restrictedAt: binding.restrictedAt ?? null,
      restrictionReason: binding.restrictionReason ?? null,
      boundAt: binding.boundAt ?? null,
      unboundAt: binding.unboundAt ?? null,
      cooperationQuality: this.cooperationQuality(binding, tasks),
    }))
  }
  ...
  private cooperationQuality(binding: MerchantAgentBinding, tasks: CreatorTask[]) {
    ...
    const related = tasks.filter(
      (task) => task.creatorId === binding.agentId && task.merchantId === binding.merchantId,
    )
    const completed = related.filter((task) =>
      ['completed', 'settled'].includes(task.status),
    ).length
    return {
      score: related.length ? Math.round((completed / related.length) * 100) : null,
      completed,
      total: related.length,
    }
  }

  private async notes(subjectType: string, subjectId: string) {
    return this.dataSource
      .getRepository(LifecycleNote)
      .find({
        where: { subjectType: subjectType as any, subjectId },
        order: { createdAt: 'DESC' },
        take: 100,
      })
  }
  private async audit(targetType: string, targetId: string) {
    return this.dataSource
      .getRepository(AuditLog)
      .find({ where: { targetType, targetId }, order: { createdAt: 'DESC' }, take: 100 })
  }
  private async requireMerchant(id: string) {
    const item = await this.dataSource.getRepository(Merchant).findOne({ where: { id } })
    if (!item) throw new NotFoundException({ code: 2002, message: '商户不存在' })
    return item
  }
  private async requireCreator(id: string) {
    const item = await this.dataSource.getRepository(SharingAgent).findOne({ where: { id } })
    if (!item) throw new NotFoundException({ code: 3004, message: '达人不存在' })
    return item
  }
  private async requireBinding(id: string) {
    const item = await this.dataSource
      .getRepository(MerchantAgentBinding)
      .findOne({ where: { id } })
    if (!item) throw new NotFoundException({ code: 6500, message: '合作关系不存在' })
    return item
  }
  private paginate<T>(items: T[], page: number, pageSize: number, total: number) {
    return { items, pagination: { page, pageSize, total, totalPages: Math.ceil(total / pageSize) } }
  }
  private normalizedTags(tags: string[]) {
    return [...new Set(tags.map((item) => item.trim()).filter(Boolean))].slice(0, 20)
  }
  private sum(rows: any[], field: string) {
    return rows.reduce((total, item) => total + Number(item[field] ?? 0), 0)
  }
  private maskPhone(phone: string) {
    return phone && phone.length >= 11 ? `${phone.slice(0, 3)}****${phone.slice(-4)}` : phone
  }
  private maskWechatId(value?: string | null) {
    return !value ? null : value.length <= 10 ? value : `${value.slice(0, 6)}…${value.slice(-4)}`
  }
  private writeAudit(
    manager: any,
    actor: Actor,
