...
export class AddBilateralCreatorTaskAppeals1790000000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE "creator_task_appeals" ADD COLUMN "merchant_id" uuid`)
    await queryRunner.query(
      `ALTER TABLE "creator_task_appeals" ADD COLUMN "appellant_type" character varying(16) NOT NULL DEFAULT 'creator'`,
    )
    await queryRunner.query(
      `ALTER TABLE "creator_task_appeals" ADD COLUMN "appeal_deadline_at" TIMESTAMP WITH TIME ZONE`,
    )
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX "idx_creator_task_appeal_merchant_status" ON "creator_task_appeals" ("merchant_id", "status")`,
    )
    await queryRunner.query(
      `ALTER TABLE "creator_task_appeals" ADD CONSTRAINT "FK_creator_task_appeal_merchant" FOREIGN KEY ("merchant_id") REFERENCES "merchants"("id") ON DELETE RESTRICT ON UPDATE NO ACTION`,
    )
  }
