...
export class AddSubscriptionPaymentOrders1789900000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "uq_subscription_payment_order_trade_no" ON "subscription_payment_orders" ("out_trade_no")`,
    )
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "idx_subscription_payment_order_merchant" ON "subscription_payment_orders" ("merchant_id", "createdAt")`,
    )
    await queryRunner.query(`
      DO $$
      BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FK_subscription_payment_order_merchant') THEN
          ALTER TABLE "subscription_payment_orders"
            ADD CONSTRAINT "FK_subscription_payment_order_merchant"
  ...
  async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "subscription_payment_orders" DROP CONSTRAINT "FK_subscription_payment_order_merchant"`,
    )
